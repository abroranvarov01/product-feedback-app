import { getCategory, getData, getQuery } from "./service.js";
...
const categoryRender = async () => {
  ...
  categoryTab.innerHTML = data
    .map(
      (item) =>
        `
      <button class="bg-btnBg rounded-[10px] py-[6px] px-[15px] font-semibold text-textColor category__btn">
           ${item}
      </button>
     `
    )
  ...
  category__btns.forEach((item) => {
    item.addEventListener("click", async () => {
      renderData(item.textContent.trim());
    });
  });
  input.addEventListener("input", async () => {
    const qData = await getQuery(input.value);
    renderData(qData);
  });
};
categoryRender();

const renderData = async (query = "all") => {
  let data;
